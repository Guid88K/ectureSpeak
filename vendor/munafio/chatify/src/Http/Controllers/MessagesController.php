...
class MessagesController extends Controller
{
    ...
    public function fetch(Request $request)
    {
        ...
        return Response::json([
            'count' => $query->count(),
            'messages' => '<p class="message-hint"><span>Скажи \'hi\' і розпочни спілкування</span></p>',
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'contacts' => $users->count() > 0 ? $contacts : '<br><p class="message-hint"><span>Ваш список контактів порожній</span></p>',
        ], 200);
    }
    ...
    public function getFavorites(Request $request)
    {
        ...
        return Response::json([
            'favorites' => $favorites->count() > 0
                ? $favoritesList
                : '<p class="message-hint"><span>Список ваших улюблених порожній</span></p>',
        ], 200);
    }
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->avatar != config('chatify.user_avatar.default')) {
                        $path = storage_path('/storage/' . config('chatify.user_avatar.folder') . '/' . Auth::user()->avatar);
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
                    ...
                    $file->storeAs('public/storage/'. config('chatify.user_avatar.folder'), $avatar);
