{{-- -------------------- Saved Messages -------------------- --}}
@if ($get == 'saved')
    <table class="messenger-list-item m-li-divider @if ('user_' . Auth::user()->id == $id && $id
        != '0') m-list-active @endif">
        <tr data-action="0">
            {{-- Avatar side --}}
            <td>
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ 'user_' . Auth::user()->id }}">Збережені повідомлення <span>Ти</span></p>
                <span>Зберігайте повідомлення таємно</span>
            </td>
        </tr>
    </table>
@endif
...
@if ($get == 'users')
    <table class="messenger-list-item @if ($user->id == $id && $id != '0') m-list-active @endif" data-contact="{{ $user->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ $type . '_' . $user->id }}">{{ $user->name }} {{ $user->surname }}
                ...
                <span>
                    ...
                    @else
                        <span class="fas fa-file"></span> Прикріплення
                    @endif
                </span>
    ...
    <table class="messenger-list-item" data-contact="{{ $user->id }}">
        <tr data-action="0">
            ...
            <td>
               {{--  <p data-id="{{ $type . '_' . $user->id }}">
                    {{ strlen($user->name) > 12 ? trim(substr($user->name, 0, 12)) . '..' : $user->name }} --}}
                    <p data-id="{{ $type . '_' . $user->id }}">{{$user->name  }} {{$user->surname}}</p>
            </td>
